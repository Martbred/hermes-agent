...
| `/side <question>` | Ask an ephemeral side question with your current conversation available as read-only context — like Codex CLI's `/side`. The question runs in a throwaway agent (the last ~20 messages are snapshotted into its prompt); the answer comes back as a panel and your main session history is untouched, so an in-flight task is never interrupted or polluted. Unlike `/background` (blank context) the side agent can see what you've been working on; unlike `/branch` you never leave your session. |
...
| `/side <question>` | Ask an ephemeral side question using the current conversation as read-only context. The answer is delivered back to the chat when ready; the main session history is never modified, and it works even while the agent is busy on another task. |
...
- `/status`, `/egress`, `/version`, `/background`, `/side`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, `/credits`, `/suggestions`, `/blueprint`, `/learn`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
